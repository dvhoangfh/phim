				...
				<form action="{{ isset($data_edit) ? route('theloai.update',$data_edit->id) : route('theloai.store') }}" method="POST" autocomplete="off">
					<div class="box-body">
					    ...
					    <div class="form-group">
						    ...
						    <input type="text" class="form-control" id="id_name" name="tentheloai" value="{{ isset($data_edit) ? $data_edit->ten_theloaiphim : '' }}" required>
						  </div>
						 <div class="form-group">
					...
					<table class="table table-bordered table-hover table-stripped">
						...
						<tbody>
							@foreach($data AS $k => $v)
								<tr>
									<td>{{$v->id}}</td>
									<td>{{$v->ten_theloaiphim}}</td>
									<td>{{$v->slug}}</td>
									<td>{{$v->stt}}</td>
									<td class="text-center">                                        
                                        <a href="{{route('theloai.edit',$v->id)}}">
                                            <span class="badge">Update</span>
                                        </a>
                                        <a href="#" data-href="{{route('theloai.delete',$v->id)}}" data-toggle="modal" data-target="#deleted">
